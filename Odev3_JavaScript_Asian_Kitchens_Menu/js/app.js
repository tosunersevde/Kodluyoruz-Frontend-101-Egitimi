...
const menuContainer = document.getElementById("item");
...
function buttonCreate() {
  ...
  menu.forEach(item => {
    if (!uniqueCategoryArray.includes(item.category)) {
      uniqueCategoryArray.push(item.category);
    }
  });
...
function createButton(category) {
  ...
  buttonDOM.addEventListener("click", () => filterClicked(category));
  buttonContainer.appendChild(buttonDOM);
}

function filterClicked(category) {
  if (category === "All") {
    showMenu(menu);
  } else {
